#!/usr/bin/env bash
# setup-k3s.sh
# Idempotent single-node k3s setup on MicroOS
# NAT-aware: auto-detects VirtualBox NAT or bridged mode
...
if rpm -q zram-generator-defaults >/dev/null 2>&1; then
  ...
  sudo transactional-update pkg remove zram-generator-defaults
  warn "Reboot required. Re-run script afterwards."
  exit 0
fi
...
overlay
EOF
for mod in br_netfilter overlay; do lsmod | grep -q "$mod" || sudo modprobe "$mod" || true; done
...
if kubectl get nodes >/dev/null 2>&1; then
  ...
  warn "k3s not ready or connectivity issue. Check: journalctl -u k3s -f"
fi
